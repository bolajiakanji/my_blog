  ...
  return (
    <CertificatitionWrapper>
      ...
      <Flex gap="7" justify="center" className="flex-wrap mb-3 mt-7">
        {cert.map((cer) => (
          <Card className="sm:max-w-96 h-40  md:max-w-80 md:mx-0 lg:max-w-96 w-full ">
            {cer}
          </Card>
        ))}
      </Flex>
      <Box className="flex justify-center ">
        {certs.length > 4 && cert.length < certs.length && (
          <Button
            onClick={handleclick}
            className={`rounded-s-md ${cert.length > 3 ? '' : 'rounded-e-md'} `}
            variant="classic"
            radius="none"
          >
            See more
        ...
        {cert.length > 6 && (
          <Button
            ...
            onClick={() => setCert(certs.slice(0, 3))}
          >
            Hide all
          </Button>
        )}
